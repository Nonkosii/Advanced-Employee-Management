...
namespace Server.Service.Repository
{
    public class UserAccount(IOptions<JwtSection> config, AppDBContext appDBContext) : IUserAccount
    {
        public async Task<GeneralResponse> CreateAsync(UserRegister user)
        {
            ...
            var checkAdminRole = await appDBContext.SystemRoles.FirstOrDefaultAsync(_ => _.Name.Equals(RoleConstants.Admin));
            if (checkAdminRole == null)
            {
                var createAdminRole = await AddToDatabase(new SystemRole() { Name = RoleConstants.Admin });
            ...
            var checkUserRole = await appDBContext.SystemRoles.FirstOrDefaultAsync(_ => _.Name.Equals(RoleConstants.User));
            SystemRole response = new();
            if (checkUserRole == null)
            {
                response = await AddToDatabase(new SystemRole() { Name = RoleConstants.User });
